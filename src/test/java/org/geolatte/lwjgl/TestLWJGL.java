...
public class TestLWJGL {
	 ...
	 */
	private static PointerBuffer demo_check_layers(
			MemoryStack stack,
	...
	private void demo_init_vk() {
		try (MemoryStack stack = stackPush()) {
				...
				if ( ip.get( 0 ) > 0 ) {
					...
					if ( requiredLayers == null ) // use alternative set of validation layers
					{
						requiredLayers = demo_check_layers(
								...
								"VK_LAYER_object_tracker", //"VK_LAYER_LUNARG_image",
								"VK_LAYER_core_validation", //"VK_LAYER_LUNARG_swapchain",
								"VK_LAYER_unique_objects"
						);
					}
				}
			...
			if ( ip.get( 0 ) != 0 ) {
				VkExtensionProperties.Buffer instance_extensions = VkExtensionProperties.mallocStack(
						ip.get( 0 ),
						stack
				);
			...
			if ( ip.get( 0 ) > 0 ) {
				VkExtensionProperties.Buffer device_extensions = VkExtensionProperties.mallocStack(
						ip.get( 0 ),
						stack
				);
	...
	private void demo_set_image_layout(
			long image,
			int aspectMask,
			int old_image_layout,
			int new_image_layout,
			int srcAccessMask) {
		try (MemoryStack stack = stackPush()) {
			if ( setup_cmd == null ) {
	...
	private boolean memory_type_from_properties(int typeBits, int requirements_mask, VkMemoryAllocateInfo mem_alloc) {
		...
		for ( int i = 0; i < VK_MAX_MEMORY_TYPES; i++ ) {
			if ( (typeBits & 1) == 1 ) {
				...
				if ( (memory_properties.memoryTypes()
						.get( i )
						.propertyFlags() & requirements_mask) == requirements_mask ) {
	...
	private void demo_prepare_depth() {
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkMemoryAllocateInfo mem_alloc = VkMemoryAllocateInfo.mallocStack( stack )
					...
					.memoryTypeIndex( 0 );
			boolean pass = memory_type_from_properties(
	...
	private void demo_prepare_pipeline() {
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkPipelineShaderStageCreateInfo.Buffer shaderStages = VkPipelineShaderStageCreateInfo.callocStack(
					2,
					stack
			);
			shaderStages.get( 0 )
